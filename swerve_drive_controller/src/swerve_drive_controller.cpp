...
namespace swerve_drive_controller {
    ...
    CallbackReturn SwerveController::on_init() {
        ...
        try {
            ...
            swerveDriveKinematics_.calculate_wheel_position(params_.wheelbase, params_.trackwidth,
    ...
    CallbackReturn
    SwerveController::on_configure(const rclcpp_lifecycle::State& /*previous_state*/) {
        ...
        try {
            ...
            // Allocate reference interfaces for chainable controller (linear.x, linear.y,
            // angular.z)
            ...
            odometry_message_.twist = geometry_msgs::msg::TwistWithCovariance(
    ...
    CallbackReturn SwerveController::on_activate(const rclcpp_lifecycle::State&) {
        ...
        for (std::size_t i = 0; i < 4; i++) {
            wheel_handles_[i] =
                get_wheel(command_interfaces_, state_interfaces_, wheel_joint_names[i]);
        }

        axle_handles_.resize(4);
        for (std::size_t i = 0; i < 4; i++) {
            axle_handles_[i] =
                get_axle(command_interfaces_, state_interfaces_, axle_joint_names[i]);
        }

        for (std::size_t i = 0; i < wheel_handles_.size(); ++i) {
            if (!wheel_handles_[i]) {
                RCLCPP_ERROR(logger, "ERROR IN FETCHING wheel handle for: %s",
                             wheel_joint_names[i].c_str());
                return CallbackReturn::ERROR;
            }
        ...
        for (std::size_t i = 0; i < axle_handles_.size(); ++i) {
            if (!axle_handles_[i]) {
                RCLCPP_ERROR(logger, "ERROR IN FETCHING axle handle for: %s",
                             axle_joint_names[i].c_str());
                return CallbackReturn::ERROR;
            }
    ...
    SwerveController::update_reference_from_subscribers(const rclcpp::Time& time,
        ...
        if (age_of_last_command > cmd_vel_timeout_) {
            ...
            reference_interfaces_[2] = 0.0;
                   ...
                   std::isfinite(command_msg_.twist.linear.y) &&
    ...
    controller_interface::return_type
    SwerveController::update_and_write_commands(const rclcpp::Time& time,
                                                const rclcpp::Duration& /*period*/) {
        auto logger = get_node()->get_logger();
        ...
        if (!std::isfinite(linear_x_cmd) || !std::isfinite(linear_y_cmd) ||
            !std::isfinite(angular_cmd)) {
        ...
        for (std::size_t i = 0; i < 4; i++) {
            double steering_error = std::abs(angles::shortest_angular_distance(
        ...
        for (std::size_t i = 0; i < 4; i++) {
            ...
            const bool is_stop = (std::fabs(linear_x_cmd) < EPS) &&
                                 (std::fabs(linear_y_cmd) < EPS) && (std::fabs(angular_cmd) < EPS);
        ...
        odometry_ = swerveDriveKinematics_.update_odometry(velocity_array, steering_angle_array,
                                                           update_dt.seconds());
    ...
    SwerveController::on_export_reference_interfaces() {
        ...
        std::vector<std::string> reference_interface_names = {"/linear/x", "/linear/y",
                                                              "/angular/z"};

        for (size_t i = 0; i < reference_interfaces_.size(); ++i) {
            reference_interfaces.push_back(hardware_interface::CommandInterface(
                get_node()->get_name() + reference_interface_names[i],
                hardware_interface::HW_IF_VELOCITY, &reference_interfaces_[i]));
        }
