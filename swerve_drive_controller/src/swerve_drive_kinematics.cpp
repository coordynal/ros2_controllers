...
namespace swerve_drive_controller {
    ...
    void SwerveDriveKinematics::calculate_wheel_position(double wheel_base, double track_width,
                                                         double x_offset, double y_offset) {
        double half_length = wheel_base / 2.0;
        double half_width = track_width / 2.0;

        wheel_positions_[0] = {half_length - x_offset,
                               half_width - y_offset}; // Front Left  (+x, +y)
        wheel_positions_[1] = {half_length - x_offset,
                               -half_width - y_offset}; // Front Right (+x, -y)
        wheel_positions_[2] = {-half_length - x_offset,
                               half_width - y_offset}; // Rear Left   (-x, +y)
        wheel_positions_[3] = {-half_length - x_offset,
                               -half_width - y_offset}; // Rear Right  (-x, -y)
    }

    std::array<WheelCommand, 4>
    SwerveDriveKinematics::compute_wheel_commands(double linear_velocity_x,
                                                  double linear_velocity_y,
                                                  double angular_velocity_z, double wheel_radius) {
    ...
    std::array<WheelCommand, 4> SwerveDriveKinematics::optimize_wheel_commands(
        ...
        for (std::size_t i = 0; i < 4; i++) {
            ...
            if (std::abs(angle_diff) > M_PI_2) {
                ...
                optimized_commands[i].drive_angular_velocity =
                    -wheel_commands[i].drive_angular_velocity;
    ...
    OdometryState
    SwerveDriveKinematics::update_odometry(const std::array<double, 4>& wheel_velocities,
        ...
        // Integrate using Exact integration for rotation, RK2 fallback for near-zero angular
        // velocity
        if (std::fabs(dtheta) < 1e-6) {
            ...
            odometry_.x +=
                (dx_robot * (sin_new - sin_old) + dy_robot * (cos_new - cos_old)) / dtheta;
            odometry_.y +=
                (dx_robot * (cos_old - cos_new) + dy_robot * (sin_new - sin_old)) / dtheta;
