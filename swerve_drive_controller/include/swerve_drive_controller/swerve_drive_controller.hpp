    ...
    class SwerveController : public controller_interface::ChainableControllerInterface {
        ...
        private:
            ...
            std::optional<T> get_interface_object(
                ...
                if (name.empty()) {
                    RCLCPP_ERROR(logger,
                                 "Joint name not given. Make sure all joints are specified.");
                    return std::nullopt;
                }

                // Use get_prefix_name() to match joint name and get_interface_name() to match
                // interface type This matches the pattern used in diff_drive_controller
                auto command_handle =
                    std::find_if(command_interfaces.begin(), command_interfaces.end(),
                ...
                if (command_handle == command_interfaces.end()) {
                    RCLCPP_ERROR(
                        logger,
                        "Unable to find command interface for: %s (expected: %s/%s, type: %s)",
                        name.c_str(), name.c_str(), hw_if_type.c_str(), hw_if_type.c_str());
                    return std::nullopt;
                }
                auto state_handle =
                    std::find_if(state_interfaces.begin(), state_interfaces.end(),
                                 [&name, &hw_if_type](const auto& interface) {
                                     return interface.get_prefix_name() == name &&
                                            interface.get_interface_name() == hw_if_type;
                                 });
                ...
                if (state_handle == state_interfaces.end()) {
                    RCLCPP_ERROR(
                        logger,
                        "Unable to find state interface for: %s (expected: %s/%s, type: %s)",
                        name.c_str(), name.c_str(), hw_if_type.c_str(), hw_if_type.c_str());
                    return std::nullopt;
                }
            ...
            inline std::optional<Wheel>
            get_wheel(std::vector<hardware_interface::LoanedCommandInterface>& command_interfaces,
                      const std::vector<hardware_interface::LoanedStateInterface>& state_interfaces,
                      const std::string& name) {
                return get_interface_object<Wheel>(command_interfaces, state_interfaces, name,
                                                   "/velocity", "velocity");
            }

            inline std::optional<Axle>
            get_axle(std::vector<hardware_interface::LoanedCommandInterface>& command_interfaces,
                     const std::vector<hardware_interface::LoanedStateInterface>& state_interfaces,
                     const std::string& name) {
                return get_interface_object<Axle>(command_interfaces, state_interfaces, name,
                                                  "/position", "position");
            }
