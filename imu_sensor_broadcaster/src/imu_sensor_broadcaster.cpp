...
namespace imu_sensor_broadcaster {
    ...
    IMUSensorBroadcaster::on_configure(const rclcpp_lifecycle::State& /*previous_state*/) {
        try {
            params_ = param_listener_->get_params();
            r_ = quat_from_euler(params_.rotation_offset.roll, params_.rotation_offset.pitch,
                                 params_.rotation_offset.yaw);
            r_.normalize();
        } catch (const std::exception& e) {
            RCLCPP_ERROR(get_node()->get_logger(),
                         "Exception thrown during config stage with message: %s \n", e.what());
            return CallbackReturn::ERROR;
        }
    ...
    controller_interface::CallbackReturn
    IMUSensorBroadcaster::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) {
        imu_sensor_->assign_loaned_state_interfaces(state_interfaces_);
        return CallbackReturn::SUCCESS;
    }

    controller_interface::CallbackReturn
    IMUSensorBroadcaster::on_deactivate(const rclcpp_lifecycle::State& /*previous_state*/) {
        imu_sensor_->release_interfaces();
        return CallbackReturn::SUCCESS;
    }

    controller_interface::return_type
    IMUSensorBroadcaster::update_and_write_commands(const rclcpp::Time& time,
    ...
    controller_interface::return_type
    IMUSensorBroadcaster::update_reference_from_subscribers(const rclcpp::Time& /*time*/,
                                                            const rclcpp::Duration& /*period*/) {
        return controller_interface::return_type::OK;
    }

    std::vector<hardware_interface::StateInterface>
    IMUSensorBroadcaster::on_export_state_interfaces() {
...
PLUGINLIB_EXPORT_CLASS(imu_sensor_broadcaster::IMUSensorBroadcaster,
